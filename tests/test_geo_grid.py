...
class TestGrillePourBbox:
    def test_grille_simple(self):
        ...
        points = grille_pour_bbox(
            sw_lat=43.20, sw_lng=2.55,
            ne_lat=44.05, ne_lng=4.20,
            pas_km=20, rayon_km=10,
        )
        assert len(points) > 0
        for p in points:
    ...
    def test_pas_plus_grand_que_zone(self):
        ...
        points = grille_pour_bbox(
            sw_lat=43.50, sw_lng=3.50,
            ne_lat=43.55, ne_lng=3.55,
            pas_km=10, rayon_km=5,
        )
        assert len(points) >= 1
    ...
    def test_bbox_invalide_leve_erreur(self):
        with pytest.raises(ValueError, match="Bbox invalide"):
            grille_pour_bbox(
                sw_lat=44.0, sw_lng=3.0,
                ne_lat=43.0, ne_lng=2.0,  # SW > NE
                pas_km=10, rayon_km=5,
            )
