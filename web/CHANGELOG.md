...
| Version | PR | Contenu |
|---|---|---|
| **1.15.0** | — | **Multi-contacts par rôle** : table `lead_contacts` (migration `0041`) + bloc « Interlocuteurs » sur la fiche (ajout, ★ principal, suppression). Le principal est recopié dans `leads.contact_*` (compat pipeline) et son **rôle adapte le ton du brouillon** : DAF → ROI/aides, DG → stratégie/APER, énergie → exploitation/technique |
