...
| Version | PR | Contenu |
|---|---|---|
| **1.17.0** | — | **Carte des tournées** : nouvel onglet 🗺️ qui place RDV (📅), prospects chauds (🔥) et leads à relancer (🟠) géolocalisés sur une carte Leaflet (auto-recadrée), avec des **opportunités groupées** — « profite de ton RDV à X pour voir les N prospects à moins de 20 km ». Helpers purs `pointsTournee`/`opportunitesAutourDesRdv` testés. Boucle la roadmap agent-first 1.12 → 1.17 |
